...
bat = bat.convert_alpha()  # aligns image format with screen pixel format
...
for y in range(brick_rows):
    ...
    for x in range(brick_cols):
        brickX = side_gap + x * (brick_rect[2] + brick_gap)
...
while not game_over:
    ...
    if (bat_rect[0] + bat_rect.width >= ball_rect[0] >= bat_rect[0]
            and ball_rect[1] + ball_rect.height >= bat_rect[1]
            and sy > 0):
        sy *= -1
        sx *= 1.15
        sy *= 1.15  # difficulty
        continue  # stop the cycle of while loop
    ...
    for b in bricks:
        bx, by = b
        if bx <= ball_rect[0] <= bx + brick_rect.width and by <= ball_rect[1] <= brick_rect.height + by:
            delete_brick = b
