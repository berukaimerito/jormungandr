...
'''
        ...
        Special case: '0' is treated as 'o'.
 '''
...
def core_logic_func(message, magazine):
    ...
    for char in message:
        ...
        if frequency_dict.get(char, None) is None:
            return False
        elif frequency_dict.get(char) == 0:
            return False
        else:
            frequency_dict[char] -= 1
            
    # if code does not return any value until here meaning that the iteration for message is completed and message can be constructed
    return True
